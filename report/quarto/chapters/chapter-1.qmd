...
This project is proposed as an application-based Software Engineering FYP. The focus is not to diagnose heart or lung disease. Instead, the focus is to design and develop a structured software prototype that can accept mixed cardiopulmonary audio, perform preprocessing, apply a machine learning-based separation method, and produce separated heart sound and lung sound outputs. Public datasets are important for this project because they allow the system to be tested and explained without relying on restricted clinical data. The HLS-CMDS dataset is especially relevant because it provides heart sounds, lung sounds, mixed recordings, and corresponding source signals recorded using a digital stethoscope and clinical manikin [@torabi2025hlscmds].

## Problem Statement

The problem statements for this project are:

1. Noise and recording artefacts reduce the quality of cardiopulmonary sound recordings, making it difficult to process the signals accurately.

2. Heart and lung sounds overlap in time and frequency when recorded together, making it difficult to separate them using traditional signal processing methods.

3. Many existing studies focus on diagnosis or isolated algorithms, while fewer works provide a reusable end-to-end software prototype that performs sound separation and evaluates the output using suitable datasets and metrics.
...
The three objectives directly address the identified problems by focusing on signal quality improvement, heart-lung sound separation, and the development of an evaluated software prototype.
...
5. Using public datasets for training, testing, or evaluation, with attention to whether the dataset provides mixed and source audio signals.
...
Chapter 3 presents the project methodology, including the development approach, data sources, preprocessing workflow, model selection, and evaluation plan.

Chapter 4 describes the system design and implementation direction, including the software architecture, major modules, data flow, storage design, model integration, and user interaction design for the prototype.

Chapter 5 presents the testing and evaluation plan, including functional testing, dataset-based testing, audio separation metrics, prototype usability testing, testing limitations, and work to be completed in the next project phase.
